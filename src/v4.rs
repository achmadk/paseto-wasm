//! PASETO V4 (Platform-Agnostic Security Tokens) Implementation
//!
//! This module provides WebAssembly bindings for PASETO version 4, which uses
//! Ed25519 for signatures and X25519 for key exchange (combined with AES-256-GCM).
//!
//! ## Overview
//!
//! PASETO is a secure token format that avoids JWT algorithm confusion vulnerabilities.
//!
//! ### V4 Specifications
//! - **Local (Encryption)**: X25519 + AES-256-GCM
//! - **Public (Signatures)**: Ed25519 (EdDSA)
//! - **Key Sizes**: 32 bytes (local/public key), 64 bytes (secret key)
//!
//! ## Token Formats
//!
//! Local: `v4.local.<base64>` | Public: `v4.public.<base64>`
...
#[wasm_bindgen]
/// Encrypt a message using PASETO V4 Local (symmetric encryption with X25519 + AES-256-GCM).
///
/// # Arguments
...
///
/// # Returns Base64-encoded token: `v4.local.<ciphertext>`
...
pub fn get_secret_key_id(secret_key_hex: &str) -> Result<String, JsValue> {
    ...
    )) // NOTE: using k4.sid. for both usage and id header
}
