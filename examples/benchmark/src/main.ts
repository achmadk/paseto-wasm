...
function renderResults(results: BenchmarkResult[]): void {
  const groups = [
    ...
    {
      title: "V4 Public Verify (Ed25519)",
      items: results.filter((r) => r.name.includes("V4 Verify")),
    },
  ];
    ...
    if (group.items.length > 0) {
      ...
      for (const item of group.items) {
        ...
        html +=
          ...
          formatOpsPerSec(item.opsPerSec) +
          '</div><div class="metric-label">ops/sec</div></div>' +
          '<div class="metric"><div class="metric-value">' +
          formatMean(item.mean) +
          '</div><div class="metric-label">mean</div></div>' +
          '<div class="metric"><div class="metric-value">±' +
          item.margin.toFixed(1) +
          '%</div><div class="metric-label">margin</div></div>' +
          "</div></div>";
      }
    }
